...
import { EventComponent, EventComponentMobile } from "./event";
...
const CalendarJadwal = ({ pegawaId }: { pegawaId: string }) => {
  const { data: session } = useSession();
  const [year, setYear] = useState(new Date().getFullYear());
  const [month, setMonth] = useState(new Date().getMonth());

  const { jadwalKerja: jadwal, fetchJadwalKerjaPegawaiByFilter: fetchJadwal } =
    useJadwalKerjaStore();

  const { dataLibur, fetchDataLibur } = useDataLiburStore();
  ...
  useEffect(() => {
    fetchDataLibur();
    fetchJadwal(pegawaId, `tahun=${year}&bulan=${month + 1}`);
  }, []);

  return (
    <>
      {/* <!-- component --> */}
      <div className="lg:flex lg:h-full lg:max-h-[60vh] lg:flex-col">
