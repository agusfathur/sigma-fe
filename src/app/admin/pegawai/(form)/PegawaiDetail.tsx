...
const PegawaiDetail = () => {
  const { pegawaiData: pegawaiDetailData } = usePegawaiStore();
  ...
  const getUser = async () => {
    try {
      const user = await axiosJWT.get(
        `${process.env.NEXT_PUBLIC_API_URL}/api/user/${pegawaiDetailData?.user_id}`,
      );
      const getPegawai = await axiosJWT.get(
        `${process.env.NEXT_PUBLIC_API_URL}/api/pegawai/user/${pegawaiDetailData?.user_id}`,
      );
  ...
  return (
    <>
      <div className="my-4 flex flex-wrap justify-center space-y-2 md:flex-nowrap md:justify-normal md:space-x-4 md:space-y-0">
        {/* Account Card */}
        <div className="h-[400px] w-[80%] rounded-lg border bg-white p-4 shadow-xl dark:bg-transparent md:w-1/3">
